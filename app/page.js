      ...
      {d && (<>
        ...
        {tab === "dept" && (<>
          {!deptData ? (
            ...
            const booked = deptData.revenue_sections?.["BOOKED REVENUE"] || deptData.revenue_sections?.["BOOKED"] || [];
            ...
            const revenueChart = (() => {
              ...
              const maxVal = Math.max(...effort.map((m) => Math.max(m.delivery, m.experiences, m.total)));
              const yScale = (v) => padT + plotH - (v / maxVal) * plotH;
              const xScale = (i) => padL + (i / (effort.length - 1)) * plotW;

              const makeLine = (key, color) => effort.map((m, i) => `${i === 0 ? "M" : "L"}${xScale(i).toFixed(1)},${yScale(m[key]).toFixed(1)}`).join(" ");
              const makeArea = (key, color, opacity) => {
                const line = effort.map((m, i) => `${i === 0 ? "M" : "L"}${xScale(i).toFixed(1)},${yScale(m[key]).toFixed(1)}`).join(" ");
              ...
              return (
                <svg viewBox={`0 0 ${chartW} ${chartH}`} style={{ width: "100%", height: "auto" }}>
                  ...
                  {/* Areas */}
                  <path d={makeArea("delivery", T.blue, 0.15)} fill="#1a2a4a" opacity={0.15} />
                  <path d={makeArea("experiences", T.teal, 0.15)} fill="#7a68a8" opacity={0.15} />
                  {/* Lines */}
                  ...
                  {effort.map((m, i) => (
                    (i % Math.max(1, Math.floor(effort.length / 10)) === 0 || i === effort.length - 1) && (
                      <text key={i} x={xScale(i)} y={chartH - 8} textAnchor="middle" fontSize={9} fill={T.textDim}>
                        {m.month.replace(/^\d{4}-/, "")}
                      </text>
                    )
                  ))}
            ...
            // Deviation chart — grouped bars like original dashboard
            const deviationChart = (() => {
              ...
              const yScale = (v) => {
                if (v >= 0) return yZero - (v / maxVal) * (plotH * 0.85);
                return yZero + (Math.abs(v) / maxVal) * (plotH * 0.1); // small room below for negatives
              };
            ...
            const utilColor = (pct) => pct >= 80 ? T.red : pct >= 60 ? T.yellow : pct >= 40 ? T.green : T.blue;
            const billableColor = (pct) => pct >= 50 ? T.green : pct >= 30 ? T.yellow : T.red;

            // Integrated projects sorted by overage desc
            ...
            return (<>
              ...
              <div className="exec-kpi-strip" style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 14, marginBottom: 24 }}>
                {[
                  { label: "Team Size", value: us.team_size, sub: `${us.high_utilization} at ≥80% utilization` },
                  { label: "Avg Utilization", value: `${us.avg_utilization}%`, color: utilColor(us.avg_utilization), sub: "Last 30 days" },
                  { label: "Avg Billable", value: `${us.avg_billable}%`, color: billableColor(us.avg_billable), sub: `${us.low_billable} below 30%` },
                  { label: "Avg Admin", value: `${us.avg_admin}%`, color: T.textMuted, sub: "Of total time" },
                  { label: rs.latest_month ? `Revenue ${rs.latest_month}` : "Latest Revenue", value: fmtK(rs.latest_total), color: T.text, sub: rs.mom_change != null ? `${rs.mom_change > 0 ? "+" : ""}${rs.mom_change}% MoM` : "—" },
              ...
              {(pen.this_month?.experiences != null || pen.this_month?.delivery != null || pen.last_month?.experiences != null) ? (
                ...
                <div style={{ background: "#fff8e1", border: `1px solid ${T.yellow}`, borderRadius: 10, padding: "12px 18px", marginBottom: 16, fontSize: 12, color: "#8d6e00" }}>
                  <strong>Penetration data:</strong> Could not extract percentages from the source sheets. Check the API response at <code>/api/dept-health</code> for debug info.
                </div>
              )}
              ...
              {(() => {
                // Look for Worked vs Booked data in revenue sections
                const worked = effort; // Total Effort = worked
                const bookedSection = deptData.revenue_sections?.["BOOKED REVENUE"] || deptData.revenue_sections?.["BOOKED"] || deptData.revenue_sections?.["BOOKED EFFORT"] || [];
                if (worked.length < 2 && bookedSection.length < 2) return null;
                // If we have both worked and booked, show the delta chart
                // If only worked, show it as a single area
                const hasBooked = bookedSection.length > 0;
                const deltaData = worked.map((w) => {
                ...
                const maxVal = Math.max(...validDelta.map((d) => Math.max(d.worked, d.booked || 0)));
                const wbH = 200;
                const xScale = (i) => padL + (i / (validDelta.length - 1)) * plotW;
                const yScale = (v) => padT + (wbH - padT - padB) - (v / maxVal) * (wbH - padT - padB);
                const makePath = (key) => validDelta.map((d, i) => `${i === 0 ? "M" : "L"}${xScale(i).toFixed(1)},${yScale(d[key] || 0).toFixed(1)}`).join(" ");
                const makeAreaPath = (key) => {
                  const line = makePath(key);
                  return `${line} L${xScale(validDelta.length - 1).toFixed(1)},${yScale(0).toFixed(1)} L${xScale(0).toFixed(1)},${yScale(0).toFixed(1)} Z`;
                };
                    ...
                    <Section title="Worked vs Booked Delta" subtitle={hasBooked ? "Area between curves = delta" : "Worked revenue only (no booked data found)"}>
                      <svg viewBox={`0 0 ${chartW} ${wbH}`} style={{ width: "100%", height: "auto" }}>
                        ...
                        {[0, 0.25, 0.5, 0.75, 1].map((f, i) => (
                          <g key={i}>
                            <line x1={padL} y1={yScale(maxVal * f)} x2={chartW - padR} y2={yScale(maxVal * f)} stroke={T.border} strokeWidth={0.5} strokeDasharray={i === 0 ? "0" : "3,3"} />
                            <text x={padL - 8} y={yScale(maxVal * f) + 4} textAnchor="end" fontSize={9} fill={T.textDim}>${(maxVal * f / 1000).toFixed(0)}K</text>
                          </g>
                        ))}
                        {/* Worked area */}
                        <path d={makeAreaPath("worked")} fill="#1a2a4a" opacity={0.15} />
                        <path d={makePath("worked")} fill="none" stroke="#1a2a4a" strokeWidth={2.5} strokeLinejoin="round" />
                        {/* Booked area */}
                        {hasBooked && <>
                          <path d={makeAreaPath("booked")} fill="#7a68a8" opacity={0.12} />
                          <path d={makePath("booked")} fill="none" stroke="#7a68a8" strokeWidth={2} strokeLinejoin="round" />
                        </>}
                        {/* Value labels on last points */}
                        {validDelta.length > 0 && (
                          <>
                            <text x={xScale(validDelta.length - 1) + 6} y={yScale(validDelta[validDelta.length - 1].worked) + 4} fontSize={9} fontWeight={700} fill="#1a2a4a">{fmtK(validDelta[validDelta.length - 1].worked)}</text>
                            {hasBooked && validDelta[validDelta.length - 1].booked != null && (
                              <text x={xScale(validDelta.length - 1) + 6} y={yScale(validDelta[validDelta.length - 1].booked) + 4} fontSize={9} fontWeight={700} fill="#7a68a8">{fmtK(validDelta[validDelta.length - 1].booked)}</text>
                            )}
                          </>
                        )}
                        {/* X-axis */}
                        {validDelta.map((m, i) => (
                          (i % Math.max(1, Math.floor(validDelta.length / 10)) === 0 || i === validDelta.length - 1) && (
                            <text key={i} x={xScale(i)} y={wbH - 4} textAnchor="middle" fontSize={9} fill={T.textDim}>{m.month}</text>
                          )
                        ))}
                        {/* Legend */}
                  ...
                  <table style={s.table}>
                    ...
                    <tbody>
                      {[...util].sort((a, b) => (b.billable || 0) - (a.billable || 0)).map((t, i) => {
                        const barW = 200;
                        const bW = Math.round(((t.billable || 0) / 100) * barW);
                        const aW = Math.round(((t.admin_time || 0) / 100) * barW);
                        const nbW = Math.max(0, Math.round((((t.utilization || 0) - (t.billable || 0) - (t.admin_time || 0)) / 100) * barW));
                        return (
                          <tr key={i} style={{ background: i % 2 === 0 ? T.bgCard : T.bgCardAlt }}>
                            ...
                            <td style={s.td}>
                              <div style={{ display: "flex", height: 14, borderRadius: 3, overflow: "hidden", width: barW, background: T.bgHover }}>
                                <div style={{ width: bW, background: T.green, transition: "width 0.3s" }} title={`Billable: ${pct(t.billable)}`} />
                                <div style={{ width: aW, background: T.yellow, opacity: 0.6, transition: "width 0.3s" }} title={`Admin: ${pct(t.admin_time)}`} />
                                <div style={{ width: nbW, background: T.blue, opacity: 0.4, transition: "width 0.3s" }} title="Other" />
                              </div>
                            </td>
                          </tr>
                        );
              ...
              <div className="chart-row" style={{ ...s.chartRow, marginBottom: 16 }}>
                ...
                <Section title="Weekly Deviation" subtitle={`Total: ${fmtK(is_.total_deviation || 0)} this week`}>
                  <div style={{ padding: "8px 0" }}>
                    {Object.entries(is_.by_ecosystem || {}).sort((a, b) => b[1].deviation - a[1].deviation).map(([eco, data]) => {
                      const maxDev = Math.max(...Object.values(is_.by_ecosystem || {}).map((d) => Math.abs(d.deviation)), 1);
                      const pctW = Math.abs(data.deviation) / maxDev * 100;
                      const isPos = data.deviation > 0;
                      return (
                        <div key={eco} style={s.barRow}>
                          ...
                          <div style={{ ...s.barValue, width: 70, color: isPos ? T.red : T.green }}>{fmtK(data.deviation)}</div>
                        </div>
                      );
                    })}
                  </div>
