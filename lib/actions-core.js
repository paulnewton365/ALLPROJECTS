...
// The Smartsheet web URL uses a hash-based identifier distinct from the
// numeric API id. Kept separate so neither breaks the other.
const REPORT_URL = process.env.ALL_PROJECTS_REPORT_URL
  || "https://app.smartsheet.com/reports/rCvpXP7mh5vRMrPwjqfQ47FVHMJpCPXWvhwp7F41";
...
// For REPORTS specifically, Smartsheet can omit empty cells from a row's
// cells array, so positional indexing mis-aligns columns downstream. Match
// by column id / virtual column id — the same pattern lib/smartsheet.js uses.
function buildColMap(columns) {
  const map = {};
  for (const col of columns || []) {
    const key = col.virtualId || col.id;
    if (key != null) map[key] = col.title;
  }
  return map;
}

function reportRowToObject(row, colMap) {
  const obj = {};
  for (const cell of row.cells || []) {
    const key = cell.virtualColumnId || cell.columnId;
    const title = colMap[key];
    if (!title) continue;
    const val = cell.displayValue != null ? cell.displayValue : cell.value;
    if (val != null) obj[title] = val;
  }
  return obj;
}
...
async function fetchProjects() {
  ...
  const colMap = buildColMap(raw.columns || []);
  const rawProjects = (raw.rows || []).map((row) => {
    const item = reportRowToObject(row, colMap);
