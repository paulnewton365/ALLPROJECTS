...
async function fetchSnapshot() {
  ...
  const nb = {
    ...
    byEcosystemStage: {}, // cross-tab: ecosystem -> { stage -> { count, forecast, weighted } }
  };

  // Cross-tab: ecosystem × request type (live work)
  const ecoRequestType = {}; // ecosystem -> { requestType -> { count, budget } }
  ...
  for (const p of projects) {
    ...
    if (segment === "live") {
      ...
      for (const t of types) {
        ...
        // Cross-tab: ecosystem × request type
  ...
  // Enriched live ecosystems with RAG + burn rate
  ...
  // Pipeline by ecosystem × stage (for stacked bar)
  const pipelineByEcosystem = Object.entries(nb.byEcosystemStage)
    .map(([eco, stages]) => ({
      ...
      stages: config.pipelineStages.map((stage) => ({
        stage,
        ...(stages[stage] || { count: 0, forecast: 0, weighted: 0 }),
      })),
    }))
    .sort((a, b) => b.total_weighted - a.total_weighted);

  // Ecosystem × request type bubble matrix
